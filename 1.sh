...
install_projectivy() {
    ...
    # [Bước 4]: Đẩy file cấu hình và sao chép dữ liệu hình nền riêng biệt hoàn toàn ngầm
    $ADB_COMMAND push projectivy.plbackup /sdcard/Download >/dev/null 2>&1
    
    # Ép đẩy toàn bộ ảnh thẳng vào thư mục gốc DCIM của TV (Không dùng hàm để tránh dính lệnh dừng Enter)
    local target_tv_dir="/sdcard/DCIM"
    for file in *.[jJ][pP][gG] *.[jJ][pP][eE][gG] *.[pP][nN][gG]; do
        [ -f "$file" ] && $ADB_COMMAND push "$file" "$target_tv_dir/" >/dev/null 2>&1
    done
    echo -e "${GREEN}✅ Đồng bộ dữ liệu hình nền và file cấu hình hoàn tất.${NC}"
...
install_X_S_2026() {
    ...
    echo -e "${GREEN}🚀 Bắt đầu cài đặt bộ giao diện Android X S A Pro 2026...${NC}"
    install_apk "p.apk" >/dev/null 2>&1
    ...
    # 💡 ĐÃ SỬA: Loại bỏ com.android.tv.settings để TV không bị lỗi đen cài đặt gốc
    ...
    # 💡 ĐÃ SỬA: Bọc khối ngầm để ẩn file đang cài tuyệt đối theo ý muốn của bạn
    for apk in $apks_to_install; do
        [ -f "$apk" ] && $ADB_COMMAND install -r -g "$apk" >/dev/null 2>&1
    done
    echo -e "${GREEN}✅ Cài đặt toàn bộ ứng dụng phụ trợ hoàn tất!${NC}"
    
    # 💡 ĐÃ SỬA: Thay đổi lệnh gọi hàm sang cơ chế push ảnh ngầm trực tiếp vào gốc DCIM không dừng màn hình
    $ADB_COMMAND push projectivy.plbackup /sdcard/Download >/dev/null 2>&1
    local target_tv_dir="/sdcard/DCIM"
    for file in *.[jJ][pP][gG] *.[jJ][pP][eE][gG] *.[pP][nN][gG]; do
        [ -f "$file" ] && $ADB_COMMAND push "$file" "$target_tv_dir/" >/dev/null 2>&1
    ...
    echo -e "  [████████████████████] 100% | ${CYAN}Cấp quyền:${NC} $pkg"
    ...
    sleep 1
    echo -e "${GREEN}✅ Đã cấp quyền và đồng bộ hóa ứng dụng!${NC}"

    # Cấu hình quyền và dịch vụ trợ năng (ĐÃ BỔ SUNG 2 DÒNG LỆNH BẺ CHẶN DNS CLOUDFLARE LÊN ĐẦU VÒNG LẶP)
    echo -e "${YELLOW}⚙️ Đang thực thi cấu hình hệ thống sâu và bẻ chặn đường truyền nhà mạng...${NC}"
    for cmd in \
    ...
    echo -e "${YELLOW}📺 TV sẽ khởi động lại để áp dụng cấu hình...${NC}"
    sleep 2
    
    # Khởi động lại TV và tự động điều hướng luồng quay về menu1 an toàn
    reboot_tv "normal"
}

# Sao chép ảnh nền vào TV (Đã tối ưu hóa đường dẫn đẩy thẳng vào gốc /sdcard/DCIM/ giữ nguyên tên file)
copy_wallpapers() {
    print_header
    echo -e "${YELLOW}🖼️ Bắt đầu chép ảnh nền (.jpg, .png) vào gốc thư mục DCIM trên TV...${NC}"
    local count=0
    
    # Sử dụng vòng lặp quét ký tự đại diện tương thích 100% với Alpine/iSH Shell
    for file in *.[jJ][pP][gG] *.[jJ][pP][eE][gG] *.[pP][nN][gG]; do
        if [ -f "$file" ]; then
            local filename=$(basename "$file")
            echo "    -> Đang chép hình: $filename"
            # Sửa đường dẫn chuẩn xác để nạp thẳng vào bộ nhớ gốc DCIM của TV Xiaomi
            $ADB_COMMAND push "$file" "/sdcard/DCIM/$filename" >/dev/null 2>&1
            count=$((count + 1))
        fi
    done

    if [ "$count" -eq 0 ]; then
        echo -e "   ${YELLOW}⚠️ Không tìm thấy file ảnh .jpg hoặc .png nào trong thư mục.${NC}"
    else
        echo -e "${GREEN}✅ Đã chép thành công $count ảnh nền vào /sdcard/DCIM/ trên TV.${NC}"
    fi
    sleep 2
}

# Cài đặt tất cả các file .apk trong thư mục nguồn (Đã sửa lỗi mảng gãy cú pháp trên iSH)
install_all_apks() {
    print_header
    echo -e "${YELLOW}🔧 Bắt đầu quét và cài đặt tất cả các file .apk...${NC}"
    echo
    ...
    if [ "$count" -eq 0 ]; then
        echo -e "   ${YELLOW}⚠️ Không tìm thấy file .apk nào để cài đặt.${NC}"
        sleep 2
        return
    fi

    # Cấu hình thiết lập mặc định cho Launcher sau khi cài app hàng loạt
    for cmd in \
    ...
    echo -e "${GREEN}✅ Đã xử lý cài đặt thành công toàn bộ $count file .apk lên TV.${NC}"
    sleep 2
    
    # Sử dụng return để giải phóng bộ nhớ RAM, luồng sẽ tự quay về menu2 điều khiển
    return 0
}
...
permission() {
    print_header
    echo -e "${YELLOW}⚙️ Đang tiến hành xin quyền hệ thống X S 2026 sau khi tắt Voice...${NC}"
    ...
    $ADB_COMMAND shell appops set com.xiaomi.voicecontrol SYSTEM_ALERT_WINDOW deny >/dev/null 2>&1
    echo -e "${GREEN}✅ Xin quyền hệ thống hoàn tất!${NC}"
    read -p "Nhấn Enter để quay lại menu chính..."
}

# Khởi động lại TV (Sửa lỗi treo luồng chạy nền dấu & trên iSH)
reboot_tv() {
    local mode="$1"
    print_header
    echo -e "${YELLOW}→ Đang gửi lệnh Reboot TV ($mode)...${NC}"
    ...
    echo -e "${GREEN}→ Đã gửi lệnh reboot thành công.${NC}"
    sleep 1

    echo -e "${YELLOW}→ Đang ngắt kết nối ADB...${NC}"
    $ADB_COMMAND disconnect >/dev/null 2>&1

    echo -e "${YELLOW}📺 TV đang khởi động lại. Tự động quay về màn hình kết nối sau 5 giây...${NC}"
    sleep 5
    
    # Thoát an toàn để trả quyền điều khiển về cho menu1 gốc, chống tràn bộ nhớ máy
    return 0
}

# =================== START ===================
# Kích hoạt hàm kiểm tra môi trường và bật menu giao diện đầu tiên
check_adb
menu1